...
from patch_index_manager import GridAlignement, GridIndexManager
from rawdataset import get_train_val_data
...
class MultiChDloader(Dataset):
    def __init__(self,
        ...
        self.set_img_sz(self.image_size,
                        self.image_size)
    ...
    def load_data(self, datasplit_type, val_fraction=None, test_fraction=None):
        self._data = get_train_val_data(self._fpath,
                                        datasplit_type,
                                        val_fraction=val_fraction,
                                        test_fraction=test_fraction,
                                        )
    ...
    def __getitem__(self, index: Union[int, Tuple[int, int]]) -> Tuple[np.ndarray, np.ndarray]:
        ...
        mean, std = self.get_mean_std()
        assert len(img_tuples) == 1
        inp = (img_tuples[0] - mean) / std
        return inp, inp
...
if __name__ == '__main__':
    ...
    dataset = MultiChDloader('/group/jug/ashesh/data/ventura_gigascience/actin-60x-noise2-lowsnr.tif', DataSplitType.Train,
                              0.1, 0.1)
    mean, std = dataset.compute_mean_std()
    dataset.set_mean_std(mean, std)
    inp, tar = dataset[0]
    print(inp.max(), inp.min(), inp.mean(), inp.std())
